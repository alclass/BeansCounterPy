...
import datetime
import os
import lib.osfs.discoverers.dateprefixdisconverers.datepathfindfs as pthfind


class DatePrefixedOSFinder:

  def __init__(self, rootfolderpath, substr=None):
  ...
  def find_files_on_date(self, pdate: datetime.date)-> list[Path]:
    try:
      return self.datedict[pdate]
    except KeyError:
      pass
    return []

  def find_files_on_refmonth(self, pdate: datetime.date | str)-> list[Path]:
    pdate = rmfs.make_refmonth_or_none(pdate)
    if pdate is None:
      return []
    try:
      return self.refmonthdict[pdate]
    except KeyError:
      pass
    return []

  def find_files_on_year(self, year: int)-> list[Path]:
    if year is None:
      return []
    try:
      return self.refmonthdict[year]
    except KeyError:
      pass
    return []
  ...
  def find_l1yyyy_folderpaths_w_opt_substr(self, substr=None):
    ...
    return: <list>
    """
    return pthfind.find_l1folderpaths_all_years_from_basefolder_opt_substr(
      self.rootfolderpath, substr
    )
  ...
  def find_l1yyyyfolderpaths_by_year_n_opt_substr(self, year, substr=None):
    return pthfind.find_l1yyyyfolderpaths_from_basefolder_w_year_opt_substr(self.rootfolderpath, year, substr)
  ...
  def find_l1yyyyfolderpath_by_year_n_opt_substr(self, year, substr=None):
    ...
    if len(paths) > 1:
      ...
      ) % (str(substr), self.rootfolderpath)
      raise ValueError(error_msg)
  ...
  def find_l2yyyymm_folderpaths_any_months_by_year_opt_substr(self, year, substr=None):
    ...
    l2yyyyfolderpaths = pthfind.find_l2yyyyfolderpaths_any_months_from_folderpath_w_year_opt_substr(
  ...
  def find_all_refmonths_l2yyyymmfolder_by_year_opt_substr(self, year, substr=None):
    ...
    for l2yyyyfolderpath in l2yyyyfolderpaths:
      ...
      refmonthdate = pthfind.derive_refmonthdate_from_a_yearmonthprefixedstr_or_mostrecent(foldername)
  ...
  def find_l2yyyymm_folderpaths_by_year_month_opt_substr(self, year, month, substr=None):
    ...
    folderpaths = pthfind.find_l2yyyymmfolderpaths_from_folderpath_w_year_month_opt_substr(
  ...
  def find_all_l2yyyymm_folderpaths(self):
    ...
    for l1_folderpath in all_l1_folderpaths:
      year = pthfind.extract_year_prefix_fr_pathsleaf(l1_folderpath)
      if year is None:
        continue
  ...
  def find_l2yyyymm_filepaths_by_year_month_typ_ext(self, year, month, typ=None, dot_ext=None):
    ...
    if l1yyyyfolderpath is None or not os.path.isdir(l1yyyyfolderpath):
      return []
    filepaths = pthfind.find_l2_or_l3_filepaths_from_folderpath_w_year_month_as_refmonth_opt_ext_substr(
  ...
  def find_l2_or_l3_filepaths_by_year_month_opt_day_ext_substr(
    ...
    Obs: substr is used for both foldernames & filenames; dot_ext is used for filenames
      (someone may have created a folder with an extension, but the system will filter it out [not osfs.isfile()])
    ...
    for folderpath in l2folderpaths:
      ...
      localfilepaths = pthfind.find_l2_or_l3_filepaths_from_folderpath_w_year_month_opt_day_ext_substr(
  ...
  def find_all_l3yyyymm_filepaths(self, dot_ext=None):
    ...
    for l2yyyymm_folderpath in all_l2yyyymm_folderpaths:
      refmonthdate = pthfind.extract_prefix_year_month_as_refmonth_from_path(l2yyyymm_folderpath)
  ...
  def __str__(self):
    outstr = 'basefolderpath = "%s"' % self.rootfolderpath
    return outstr
