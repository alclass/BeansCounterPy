...
class AutoFindCommaPointNumberTransformer:
  ...
  Noticing:
    Output:
      a) a pandasfs to a point-sep number will also make it a float (instead of str)
      b) a pandasfs to a comma-sep number will leave it as str
    Input:
  ...
  def find_out_it_is_a_commadecplacenumber_or_raise(self):
    ...
    if '.' in strnumber:
      ...
      if ',' not in strnumber:
        ...
        if not isinstance(strnumber, float):
          errmsg = (f"Error: cannot pandasfs '{strnumber}' to point number"
                    f" due to ambiguity in its dots or commas")
          raise ValueError(errmsg)
    # count dots
    dots = list(filter(lambda c: c == '.', strnumber))
    if len(dots) > 1:
      errmsg = (f"Error: cannot pandasfs comma number {strnumber} to point number"
    ...
    if not bool_res:
      errmsg = (f"Error: cannot pandasfs comma number {strnumber} to point number"
                f" due to it not being formed under the convention numberfs comma plus optional numberfs")
      raise ValueError(errmsg)
    # right-strip commas (to avoid numberfs ending with comma and no more numberfs following)
  ...
  def treat_dot_number_fr(self):
    try:
      ...
      errmsg = (f"Error: cannot pandasfs point number {self.number_fr} to comma str number"
...
def step1_remove_thousands_point_sep_in_numbers_via_re(input_text):
  """
  Step 1: remove all thousands points in numberfs
...
def step2_convert_commasep_to_pointsep_in_numbers_via_re(input_text):
  """
  Step 2: replace all comma decimal points to point ones in numberfs
