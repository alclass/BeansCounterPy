...
import lib.fncfs.indices.ipca as init  # init.get_ipca_datadir()
...
def fetch_ipca_for_years(yearini=2025, yearfin=2026):
  for year in range(yearini, yearfin):
    fetch_n_store_monthly_ipcas_to_jsonfile_fo_year(year)
...
def fetch_n_store_monthly_ipcas_to_jsonfile_fo_year(year: int):
  ...
  today = datetime.date.today()
  if year == today.year:
    return fetch_n_store_ipcas_in_current_year_uptilnow()
...
def get_year_monthly_ipcas_pct_via_jsonfile(year: int) -> dict[datetime.date, decimal.Decimal] | None:
  """
  lib.fncfs.indices.ipca.ipca_api_fetcher.read_n_get_json_ipca_monthlyindices_via_file_for_year
...
def adhoctest1():
  ...
  scrmsg = f"""{__name__} | IPCA BCB web-API fetching:
  from: {inidate} to {findate}"""
  print(scrmsg)
