#!/usr/bin/env python3
"""
art/bnk/inst/bb/downloadTabelaBBRendimentosDia.py
...
import art.finc.bnk.models.bb.fi.bbfi_file_find as ffnd  # for ffnd.BBFIFileFinder.Props.commapoint_htmlfilename_to_interpol
import art.finc.bnk.models.bankpathfinder as bkfnd  # .BankOSFolderFileFinder
import art.finc.bnk.models.bb.fi.fibb_daily_results_numbers_comma_to_point_convert as commapoint  # .SingleFileConverter
import art.finc.bnk.models.bb.fi.fibb_daily_results_html_to_csv_via_pandas_transform as transf  # .WithPandasHtmlToCsvConverter
...
class BBRendDiaDownloader:
  ...
  def mount_daybased_targetfilepath(self):
    """
    fs.osfs.dateprefixed_dirtree_finder.DatePrefixedOSFinder
  ...
  def target_folderpath(self):
    ...
    if not os.path.isdir(self._targetfolderpath):
      ...
      osfs.makedirs(self._targetfolderpath)
  ...
  def download_in_currentdir(self):
    """
    This option calls osfs.system() issuing command "wget".
    ...
    osfs.rename('index.jsp?tipo=01', table_filename)
  ...
  def download_to_targetdir(self):
    ...
    The other one calls osfs.system() issuing command "wget".
...
def download_n_gen_csv(pdate=None):
  ...
  if idate == today:
    print("Step 1 download HTML rendimentos no dia (original has comma decimal-place numberfs)")
