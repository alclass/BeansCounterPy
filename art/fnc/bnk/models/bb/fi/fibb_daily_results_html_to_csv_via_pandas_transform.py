#!/usr/bin/env python3
"""
pydantmodels/pydantmodels/bb/fi/fibb_daily_results_html_to_csv_via_pandas_transform.py
"""
import datetime
import os
import shutil
import pandas as pd
import art.finc.bnk.models.bb.fi.fibb_daily_results_numbers_comma_to_point_convert as commapoint
...
class WithPandasHtmlToCsvConverter:
  ...
  def treat_date(self):
    if not isinstance(self.date, datetime.date):
      ...
      if not isinstance(self.date, datetime.date):
        error_msg = 'Error: program could not pandasfs input date [%s] to object date' % str(self.date)
