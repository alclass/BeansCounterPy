...
class FundoAplic:
  ...
  def __init__(self):
    """
    UNIQUE(bank_3letter, name, refmonthdate)
    """
  ...
  def sync_refmonthdate_if_needed(self):
    if self.refmonthdate is None:
      if self.data_saldo_atu is None:
        ...
        if pdate is None:
          error_msg = 'Error: data_fim_cota (%s) could not be pandasfs into a formal date type.' % self.data_saldo_atu
          raise ValueError(error_msg)
        self.data_saldo_atu = pdate
        self.refmonthdate = rmfs.make_refmonth_or_none(pdate)
