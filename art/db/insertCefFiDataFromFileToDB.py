...
import lib.dbfs.dbasfolder.find_dateprefixedfiles_fr_basefolder as lkup  # for finding "conventioned" paths
import lib.osfs.discoverers.dateprefixdisconverers.datepathfindfs as hilo  # for recuperating refmonth from str
import art.finc.bnk.models.banksgeneral as bkge  # for bootsrapping bnk's fi's base folderpath (needed for lkup)
import art.finc.bnk.models.bb.fi.bbScraperWithFileText as bbScrp  # for the BBFI text scraper
import art.finc.bnk.models.fundoAplicSql as fApSql  # for the "fundo" class composing class here for db-inserting
...
class CefFiDataFromFilesToDBLoader:
  ...
  def get_fundo_from_datafile_by_filepath(self, filepath):
    ...
    fundo.bank3letter = self.bank3letter  # notice the 'coupling' of bnk is outside scraping
    return fundo
