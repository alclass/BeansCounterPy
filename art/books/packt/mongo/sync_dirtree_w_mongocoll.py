...
"""
import sys
import art.books.packt.dirwalk.dirWalkPacktBookInfoExtractor as bkExtr
...
class SyncBookDirTreeWithMongoColl:
  ...
  def is_book_in_db(self, bookinfo):
    ...
    At the same time, some books did not have an ISBN at
    ...
    if bookinfo.isbn is not None:
      ...
      if docsize == 0:
        ...
        print('ISBN just one:', bookinfo.isbn)
        return
      else:
  ...
  def delete_excess(self, bookinfo, ldocs, docsize):
    scrmsg = f"ISBN {docsize} repeats: {bookinfo.isbn}"
    print(scrmsg)
    scrmsg = f"{bookinfo}"
    print(scrmsg)
    print('-'*40)
    scrmsg = f"docsize {docsize} | {bookinfo.isbn}"
    print(scrmsg)
    if docsize > 1:
  ...
  def __str__(self):
    ...
    number of books in folders = {self.n_books_in_dirtree}
    number of books in db = {self.n_books_in_db}
    number of books without isbn = {self.n_books_w_null_isbn}
