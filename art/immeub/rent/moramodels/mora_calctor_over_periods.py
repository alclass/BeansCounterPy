#!/usr/bin/env python3
"""
art/immeub/rent/moramodels/mora_calctor_over_periods.py
...
class MoraOPeriodsCalculator(pydantic.BaseModel):
  """
  This class calculates 'mora' over periods.

  There is another class, called SameMonthMora,
    which is used for calculated the intramonth mora.
  This intramonth mora is designed, in the context
    of a rent contract as defined in here, to create
    a billing-item in the following month's charge bill.

  This clas should be considered immutable in the sense
    that is a once-processor,
  ...
  findate: datetime.date
  fix_ir_pct: Decimal
  var_ir_pct: Decimal
  _finalmontant: Decimal
  _mora_increment: Decimal
  _days_elapsed: int
  _months_elapsed: int
  _multfactor_for_fm: Decimal
